...
public record SyncSettingsS2CPayload(HudVisibility hud, KeybindSettings keybinds) implements CustomPayload {
    ...
    public static final PacketCodec<net.minecraft.network.RegistryByteBuf, SyncSettingsS2CPayload> CODEC =
            PacketCodec.tuple(
                    PacketCodecs.BOOL, p -> p.hud().visible(),
                    PacketCodecs.BOOL, p -> p.keybinds().enabled(),
                    (hudVisible, keybindEnabled) ->
                            new SyncSettingsS2CPayload(new HudVisibility(hudVisible), new KeybindSettings(keybindEnabled))
            );
